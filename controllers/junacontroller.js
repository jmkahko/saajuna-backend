  ...
  haeAikataulu: (req, res) => {
    ...
    request(options, function (error, response) {
      ...
      if (response.statusCode === 200) {
        ...
        junanaikataulu.push(['startStation', data[0]['timeTableRows'][0]['stationShortCode']])

        // Junan viimeinen asema
        let timeTableRiveja = data[0]['timeTableRows'].length - 1;
        junanaikataulu.push(['endStation', data[0]['timeTableRows'][timeTableRiveja]['stationShortCode']])
        ...
        for (let x = 0; x < data[0]['timeTableRows'].length; x++) {
          ...
          aikataulu.push(['station', data[0]['timeTableRows'][x]['stationShortCode']]);
